...
<script type="text/javascript">
	$(function(){
	    ...
	    $.ajax({
	    	...
	    	success : function(data){
	    		...
	    		$('#log_checkldl_checkldlselect').append("<option value='0' selected='true'>请选择日期</option>");
	    		var mdate = obj.dateValue;
	    		//console.info("get mdate:"+mdate);
	    		$.each(mdate,function(i){
	    			//console.info("this json:"+mdate[i]);
	    			$('#log_checkldl_checkldlselect').append($("<option/>").text(mdate[i].logTime.toString().substring(0,10)).attr("value",mdate[i].logTime.toString().substring(0,10)));
	    		})
	    }});
	}
	);
	 $('#log_checkldl_checkldlselect').change(function(){
		 if($('#log_checkldl_checkldlselect option:selected').val()!=0){
			 getcheckldlDataGrid();
     	}else{
	...
	function getcheckldlDataGrid(){
		$('#log_checkldl_checkldldatagrid').datagrid({
			...
			queryParams:{
				"logType":'check',
				"logTime":$('#log_checkldl_checkldlselect option:selected').text()
    		},
			frozenColumns : [ [ {
				field : 'analyName',
				title : '彩种',
 ...
 <form id="log_checkldl_checkldlform" method="post">
 	各彩种平均用时:<select id="log_checkldl_checkldlselect" ></select>
 </form>
