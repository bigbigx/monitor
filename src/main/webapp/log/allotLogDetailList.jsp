...
<script type="text/javascript">
	$(function(){
	    ...
	    $.ajax({
	    	...
	    	success : function(data){
	    		...
	    		$('#log_allotldl_allotldlselect').append("<option value='0' selected='true'>请选择日期</option>");
	    		var mdate = obj.dateValue;
	    		//console.info("get mdate:"+mdate);
	    		$.each(mdate,function(i){
	    			//console.info("this json:"+mdate[i]);
	    			$('#log_allotldl_allotldlselect').append($("<option/>").text(mdate[i].logTime.toString().substring(0,10)).attr("value",mdate[i].logTime.toString().substring(0,10)));
	    		})
	    }});
	}
	);
	 $('#log_allotldl_allotldlselect').change(function(){
		 if($('#log_allotldl_allotldlselect option:selected').val()!=0){
			 getallotldlDataGrid();
     	}else{
     		$('#log_allotldl_allotldldatagrid').datagrid('loadData', { total: 0, rows: [] });
     	}
     		
     });
	function getallotldlDataGrid(){
		$('#log_allotldl_allotldldatagrid').datagrid({
			...
			queryParams:{
				...
				"logType":'allot',
				"logTime":$('#log_allotldl_allotldlselect option:selected').text()
    		},
			frozenColumns : [ [ {
				field : 'analyName',
				title : '彩种',
 ...
 <p>请选择时间展现彩种分布情况:</p>
 <form id="log_allotldl_allotldlform" method="post">
 	各彩种平均用时:<select id="log_allotldl_allotldlselect" ></select>
 </form>
<table id="log_allotldl_allotldldatagrid"></table>